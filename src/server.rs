...
async fn handle_redis_command_inner(
    ...
    match command {
        ...
        RedisCommand::Limiter { key, value } => {
            handle_limiter(stream, state, key, value).await?;
        }
...
async fn handle_limiter(
    stream: &mut TcpStream,
    state: &Arc<AppState>,
    key: String,
    value: Option<u64>,
) -> Result<(), Box<dyn std::error::Error>> {
    let l = if let Some(v) = value {
        let message = limiter::LimiterMessage::SetLimit {
            api_key: key.clone(),
            limit: v,
        };
        if let Err(e) = state.limiter.tx.send_async(message).await {
            tracing::error!("send limiter message for key {}: {}", key, e);
        }
        v
    } else {
        let (tx, rx) = oneshot::channel();
        let message = limiter::LimiterMessage::GetLimit {
            api_key: key.clone(),
            tx,
        };
        if let Err(e) = state.limiter.tx.send_async(message).await {
            tracing::error!("send limiter message: {}", e);
        }
        match rx.await {
            Err(e) => {
                tracing::error!("receive limit for key {}: {}", key, e);
                0
            }
            Ok(l) => l,
        }
    ...
    let response = BytesFrame::Integer(l as i64);
    stream.write_all(&serialize_frame(&response)).await?;
    Ok(())
}
