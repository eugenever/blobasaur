...
pub struct AppState {
    ...
    /// separate connection pool for writers with pool_max_connections = 1
    pub db_pools_writers: Vec<SqlitePool>,
...
impl AppState {
    pub async fn new(
        ...
        fs::create_dir_all("limiters").expect("Failed to create limiters directory");
        ...
        let mut db_pools_writers_futures = vec![];
        ...
        let db_pool_writers_results: Vec<Result<SqlitePool, sqlx::Error>> =
            join_all(db_pools_writers_futures).await;
        let db_pools: Vec<SqlitePool> = db_pool_results
            ...
            .collect();
        let db_pools_writers: Vec<SqlitePool> = db_pool_writers_results
            .into_iter()
            .enumerate()
            .map(|(i, res)| {
                res.unwrap_or_else(|e| panic!("Failed to connect to shard {} DB: {}", i, e))
        ...
        let (_jh_limiter, _jh_save) =
            limiter::limiter_task(tx.clone(), rx.clone(), cfg.limiter.clone())?;
        let l = Limiter { rx, tx };
        if let Err(e) = l.tx.send_async(limiter::LimiterMessage::Load).await {
