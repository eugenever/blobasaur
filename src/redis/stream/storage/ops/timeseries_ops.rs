...
use crate::redis::stream::{
    ...
    storage::{
        ...
        types::{Aggregation, CompactionRule, DataType, DuplicatePolicy, Entry, TimeSeriesInfo},
    },
};
...
impl Store {
    ...
    pub fn ts_create(
        ...
        match self.data_entry(&key) {
            storage::dashtable::Entry::Occupied(_) => Err(Error::Other("TSDB: key already exists")),
    ...
    pub fn ts_incrby(&self, key: Bytes, value: f64, timestamp: Option<i64>) -> Result<i64> {
        let ts_val = timestamp.unwrap_or_else(storage::value::now_ms);
