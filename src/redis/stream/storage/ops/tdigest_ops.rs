...
use crate::redis::stream::storage::{self, Store, tdigest::TDigest, types::DataType};
...
impl Store {
    ...
    pub fn tdigest_create(&self, key: &Bytes, compression: f64) -> Result<bool, &'static str> {
        ...
        let entry = storage::Entry::new(DataType::TDigest(Box::new(td)));
    ...
    pub fn tdigest_add(&self, key: &Bytes, values: &[f64]) -> Result<(), &'static str> {
        match self.data_entry(key) {
            ...
            storage::dashtable::Entry::Vacant(_) => Err("ERR T-Digest: key does not exist"),
        }
    }
