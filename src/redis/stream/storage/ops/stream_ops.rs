...
use crate::redis::stream::{
    ...
    storage::{
        self, Store,
        types::{Consumer, ConsumerGroup, DataType, Entry, PendingEntry, StreamData, StreamId},
        value::now_ms,
    },
};
...
impl Store {
    ...
    pub fn xdel(&self, key: &[u8], ids: &[StreamId]) -> Result<usize> {
        match self.data_entry(key) {
            ...
            storage::dashtable::Entry::Vacant(_) => Ok(0),
        }
    }
    ...
    pub fn xgroup_createconsumer(&self, key: &[u8], group: &[u8], consumer: Bytes) -> Result<bool> {
        match self.data_entry(key) {
            ...
            storage::dashtable::Entry::Vacant(_) => Err(Error::Stream(
                "ERR The XGROUP subcommand requires the key to exist".into(),
            )),
        }
    }
    ...
    pub fn xgroup_destroy(&self, key: &[u8], group: &[u8]) -> Result<bool> {
        match self.data_entry(key) {
            ...
            storage::dashtable::Entry::Vacant(_) => Ok(false),
        }
    }
    ...
    pub fn xreadgroup(
        ...
        for (key, id_bytes) in keys.iter().zip(ids.iter()) {
            ...
            match self.data_entry(key.as_ref()) {
                ...
                storage::dashtable::Entry::Vacant(_) => {}
            }
        }
    ...
    pub fn xclaim(
        ...
        match self.data_entry(key) {
            ...
            storage::dashtable::Entry::Vacant(_) => Ok(vec![]),
        }
    }
    ...
    pub fn xautoclaim(
        ...
        match self.data_entry(key) {
            ...
            storage::dashtable::Entry::Vacant(_) => Ok(AutoClaimResult {
                next_start_id: StreamId::ZERO,
                claimed: vec![],
                deleted_ids: vec![],
            }),
    ...
    pub fn xsetid(
        ...
        match self.data_entry(key) {
            ...
            storage::dashtable::Entry::Vacant(_) => Err(Error::Custom("ERR no such key".into())),
        }
    }
}
