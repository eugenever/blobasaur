...
impl Store {
    ...
    fn create_set_data(
        ...
        if all_ints && int_members.len() <= max_intset_entries {
            let mut intset = storage::intset::IntSet::new();
    ...
    pub fn srem(&self, key: &[u8], members: &[Bytes]) -> usize {
        match self.data_entry(key) {
            ...
            storage::dashtable::Entry::Vacant(_) => 0,
        }
    }
    ...
    pub fn spop(&self, key: &[u8], count: usize) -> (Vec<Bytes>, bool) {
        match self.data_entry(key) {
            ...
            storage::dashtable::Entry::Vacant(_) => (vec![], false),
        }
    }
                ...
                match &entry_ref.1.data {
                    ...
                    DataType::SetPacked(lp) => {
                        ...
                        for member in &all_members[start..end] {
                            let matches =
                                pattern.map_or(true, |p| storage::match_pattern(p, member));
                            if matches {
    ...
    fn scan_dashset(
        ...
        use storage::dashtable::calculate_hash;
        ...
        loop {
            ...
            for member_ref in set.iter() {
                ...
                if bucket == (current_cursor & mask) {
                    let matches = pattern.map_or(true, |p| storage::match_pattern(p, member));
                    if matches {
                        result.push(member.clone());
                    }
                }
