...
use crate::redis::stream::{
    ...
    storage::{
        self, Store,
        types::{DataType, Entry, VectorNode, VectorQuantization, VectorSetData},
    },
};
...
impl Store {
    ...
    pub fn vadd(
        ...
        match self.data_entry(&key) {
            storage::dashtable::Entry::Vacant(e) => {
                ...
                if quant == VectorQuantization::Q8 {
                    vs.q8_stats = Some(storage::types::Q8GlobalStats::from_vector(&final_vector));
                }
            ...
            storage::dashtable::Entry::Occupied(mut e) => {
                ...
                if expected_input_dim != 0 && expected_input_dim != original_dim {
                    ...
                    return Err(crate::redis::stream::error::Error::Custom(
                        if vs.original_dim.is_some() {
                ...
                if quant == VectorQuantization::Q8 {
                    if let Some(ref mut stats) = vs.q8_stats {
                        ...
                        vs.q8_stats =
                            Some(storage::types::Q8GlobalStats::from_vector(&final_vector));
                    }
                }
    ...
    pub fn vsim(
        ...
        filter: Option<&str>,
        epsilon: Option<f32>,
    ) -> crate::redis::stream::error::Result<Vec<(Bytes, Option<f32>, Option<String>)>> {
        ...
        let query_vec: std::borrow::Cow<'_, [f32]> = if let Some(original_dim) = vs.original_dim {
            ...
            if query.len() != vs.dim {
                return Err(crate::redis::stream::error::Error::Custom(format!(
                    "ERR Vector dimension mismatch. Expected {}, got {}",
                    vs.dim,
                    query.len()
                )));
    ...
    pub fn vrem(&self, key: &[u8], element: &[u8]) -> bool {
        match self.data_entry(key) {
            ...
            storage::dashtable::Entry::Vacant(_) => false,
        }
    }
