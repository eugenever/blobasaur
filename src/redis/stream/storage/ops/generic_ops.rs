...
use crate::redis::stream::storage::{
    self, Store,
    dashtable::{DashTable, calculate_hash},
    types::{DataType, Entry, SortedSetData, StreamData, VectorSetData},
};
            ...
            DataType::VectorSet(vs) => {
                let new_vs = VectorSetData {
                    ...
                    nodes: vs
                        .nodes
                        .iter()
                        .map(|n| storage::types::VectorNode {
    ...
    pub fn restore_key(
        ...
        let data_type = match type_byte {
            ...
            10 => {
                ...
                let bf = storage::bloomfilter::ScalableBloomFilter::from_bytes(bf_data)
            ...
            11 => {
                ...
                let cf = storage::cuckoofilter::ScalableCuckooFilter::from_bytes(cf_data)
        ...
        let final_expire = if absttl {
            ...
            Some(storage::value::now_ms() + ttl_ms)
