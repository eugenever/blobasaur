...
use crate::redis::stream::storage::Store;
use crate::redis::stream::storage::StreamId;
use crate::redis::stream::storage::ops::stream_ops::TrimStrategy;
...
fn cmd_xgroup(store: &Store, args: &[Bytes]) -> Result<RespValue> {
    ...
    //     let s = String::from_utf8(e.to_vec());
    //     println!("s: {:?}", s);
    // });
    if args.is_empty() {
