...
impl RedisCommand {
    ...
    pub fn name(&self) -> String {
        match self {
            ...
            RedisCommand::Limiter { .. } => "LIMITER".to_string(),
...
fn parse_command_array(elements: Vec<BytesFrame>) -> Result<RedisCommand, ParseError> {
    ...
    match command_name.as_str() {
        ...
        "LIMITER" => {
            // Get current limit by API Key
            // LIMITER b9da27e2-8440-4561-8a09-c0c9781410a1

            // Set limit for API Key
            // LIMITER b9da27e2-8440-4561-8a09-c0c9781410a1 VALUE 1250

            if elements.len() != 2 && elements.len() != 4 {
                return Err(ParseError::Invalid(
                    "LIMITER requires 2 or 4 arguments".to_string(),
                ));
            }
            let key = extract_string(&elements[1])?;
            ...
            let mut value = None;
            if elements.len() == 4
                && let Some(v) = elements.get(2)
                && extract_string(&v)?.to_uppercase() == "VALUE"
            {
                value = Some(
                    extract_string(&elements[3])?
                        .parse::<u64>()
                        .map_err(|e| ParseError::Invalid(e.to_string()))?,
                );
            }
            Ok(RedisCommand::Limiter { key, value })
        }
        "CLUSTER" => {
            if elements.len() < 2 {
                return Err(ParseError::Invalid(
