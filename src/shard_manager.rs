            ...
            | ShardWriteOperation::HSetAsync {
                    ...
                    ensure_namespaced_table_exists(&mut tx, &table_name, known_tables).await
                {
                    ...
                    if exists {
                        ...
                        loop {
                            if attempts >= COUNT_RETRY {
                                break;
                            }
                            sqlite_res = sqlx::query(&update_query)
                                .bind(&data[..])
                                .bind(now)
                                .bind(key)
                                .execute(&mut *tx)
                        ...
                        sqlite_res.map_err(|e| {
                            tracing::error!(
                                "[Shard {}] HSET UPDATE error for namespace {} key {}, attempts {}: {}",
                                shard_id,
                                namespace,
                                key,
                                attempts,
                        ...
                        loop {
                            ...
                            sqlite_res = sqlx::query(&insert_query)
                                .bind(key)
                                .bind(&data[..])
                                .bind(now)
                                .bind(now)
                        ...
                        sqlite_res.map_err(|e| {
                            tracing::error!(
                                "[Shard {}] HSET INSERT error for namespace {} key {}, attempts {}: {}",
                                shard_id,
                                namespace,
                                key,
                                attempts,
